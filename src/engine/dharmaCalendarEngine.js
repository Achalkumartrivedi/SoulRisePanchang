...
const festivalRepository_1 = require("./festivalRepository");
const jainCalendarEngine_1 = require("./jainCalendarEngine");
const panchangEngine_1 = require("./panchangEngine");
const vedicTerms_1 = require("../i18n/vedicTerms");
/**
 * Calculates authentic calendar details for any date and selected calendar tradition.
 */
function getDharmaCalendarDayData(date, calendarSystem, language = 'hi', lunarSystem = 'PURNIMANTA') {
    const dateIso = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
    const gDay = date.getDate();
    const gMonth = date.getMonth(); // 0-indexed
    const gYear = date.getFullYear();
    // Find festival matches in repository
    const exactFestivalMatch = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso);
    switch (calendarSystem) {
        ...
        case 'SIKH': {
            ...
            const sikhFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'SIKH_FESTIVAL');
            return {
                calendarSystem: 'SIKH',
        ...
        case 'BUDDHIST': {
            ...
            const tithiIdx = (0, panchangEngine_1.calculateTithiForDate)(date);
            const isFullMoon = tithiIdx === 14;
            const isNewMoon = tithiIdx === 29;
            const buddhistFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'BUDDHIST_FESTIVAL');
            let badgeText = undefined;
            if (buddhistFest)
                badgeText = `☸️ ${buddhistFest.name.split(' ')[0]}`;
            else if (isFullMoon)
        ...
        case 'CHRISTIAN': {
            ...
            const christianFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'CHRISTIAN_FESTIVAL');
            return {
        ...
        case 'PARSI': {
            ...
            const parsiFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'PARSI_FESTIVAL');
            return {
        ...
        case 'GLOBAL': {
            ...
            const worldFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'WORLD_FESTIVAL');
            return {
        ...
        case 'JAIN': {
            const tithiIdx = (0, panchangEngine_1.calculateTithiForDate)(date);
            const jData = (0, jainCalendarEngine_1.getJainDayData)(date, tithiIdx);
            const jainFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category === 'JAIN_FESTIVAL');
            let badgeText = undefined;
            if (jainFest)
                badgeText = `🪔 ${jainFest.name.split(' ')[1] || 'Parva'}`;
            else if (jData.isParvaTithi)
                badgeText = `🪔 ${jData.parvaType?.split(' ')[1] || 'Parva'}`;
            return {
                calendarSystem: 'JAIN',
        ...
        default: {
            const tithiIdx = (0, panchangEngine_1.calculateTithiForDate)(date);
            const tithiName = (0, vedicTerms_1.getLocalizedTithi)((tithiIdx % 15) + 1, language).name;
            const hinduMonthName = (0, panchangEngine_1.getHinduMonthName)(date, lunarSystem);
            const pakshaFull = (0, vedicTerms_1.getLocalizedPakshaName)(tithiIdx <= 14 ? 'SHUKLA' : 'KRISHNA', language);
            const hinduFest = festivalRepository_1.FESTIVALS.find(f => f.dateIso === dateIso && f.category !== 'JAIN_FESTIVAL' && f.category !== 'WORLD_FESTIVAL');
            let badgeText = undefined;
            if (tithiIdx === 14)
