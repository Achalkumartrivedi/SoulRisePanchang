...
exports.getJulianDayUTC = getJulianDayUTC;
exports.calculateSolarLunarPositions = calculateSolarLunarPositions;
...
const muhuratCalculator_1 = require("./muhuratCalculator");
...
function calculateSolarLunarPositions(date) {
    ...
    const moonPerturbations = 6.288774 * Math.sin(Mm_rad) +
...
const getHinduMonthName = (d, lunarSystem = 'PURNIMANTA') => {
...
const getPerpetualMiniRitual = (d, tithiIdx, monthName) => {
...
function getTimezoneOffsetMinutes(timeZoneId = 'Asia/Kolkata', date = new Date()) {
    const tzMap = {
...
function findLimbStartTime(sunriseUtcDate, tzOffsetMin, tzAbbrev, getLimbIndex) {
    const initialIndex = getLimbIndex(sunriseUtcDate);
    const stepMinutes = 15;
    const maxMinutes = 1800;
    let transitionTimeMs = null;
...
function findLimbEndTime(sunriseUtcDate, tzOffsetMin, tzAbbrev, getLimbIndex) {
    const initialIndex = getLimbIndex(sunriseUtcDate);
    const stepMinutes = 15;
    const maxMinutes = 1800;
    let transitionTimeMs = null;
...
function calculatePanchang(date, city, lunarSystem = 'AMANTA') {
    ...
    if (sunriseParts[1] === 'PM' && h < 12)
        h += 12;
    if (sunriseParts[1] === 'AM' && h === 12)
        h = 0;
    ...
    const { auspicious, inauspicious } = (0, muhuratCalculator_1.calculateMuhurats)(date, sunrise, sunset);
    const { dayChoghadiya, nightChoghadiya } = (0, muhuratCalculator_1.calculateChoghadiya)(date, sunrise, sunset);
    const festivals = (0, festivalRepository_1.getFestivalsForDate)(dateIso);
...
function calculateSunriseSunset(date, lat, lon, timeZoneId = 'Asia/Kolkata') {
